    ...
    render() {
        ...
        panel.innerHTML = `
            ...
            <div style="width:100%; overflow-y:auto; flex-grow:1; display:flex; flex-direction:column; gap:10px; padding: 0 24px; box-sizing: border-box;">
                ...
                ${this.currentOpenSection === 'words' ? `<div class="stats-list" style="padding:0 10px 10px 10px;">${words.map(([w, c]) => `<div class="history-list-item" onclick="window.AppSearch('${UIUtils.escapeJS(w)}');"><div class="history-word-info"><span style="font-weight:600;">${w}</span><span class="history-word-count">(${c}x, ${this.getFormattedTime(this.stats.wordTime[w])})</span></div><button class="remove-history-btn" onclick="window.promptHomeRemoval(this, '${UIUtils.escapeJS(w)}', 'word', event)">&times;</button></div>`).join('')}</div>` : ''}
                ...
                ${this.currentOpenSection === 'tags' ? `<div class="stats-list" style="padding:0 10px 10px 10px;">${tags.map(([t, c]) => `<div class="history-list-item" onclick="window.ModalManager.show('${UIUtils.escapeJS(t)}');"><div class="history-word-info"><span style="font-weight:600;">${t}</span><span class="history-word-count">(${c}x, ${this.getFormattedTime(this.stats.tagTime[t])})</span></div><button class="remove-history-btn" onclick="window.promptHomeRemoval(this, '${UIUtils.escapeJS(t)}', 'tag', event)">&times;</button></div>`).join('')}</div>` : ''}
            </div>
