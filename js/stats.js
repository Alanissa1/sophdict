window.StatsManager = {
    ...
    togglePanel() {
        const panel = document.getElementById('statsPanel');
        if (panel && panel.style.display === 'flex') this.hide();
        else this.show();
    },
    ...
    render() {
        ...
        panel.innerHTML = `
            <div class="stats-header"><span class="stats-title">Usage Statistics</span><button class="stats-close-btn" onclick="StatsManager.hide()">&times;</button></div>
            ...
            <div style="width:100%; overflow-y:auto; flex-grow:1; display:flex; flex-direction:column; gap:10px; padding: 0 24px; box-sizing: border-box;">
                ...
                ${this.currentOpenSection === 'words' ? `<div class="stats-list" style="padding:0 10px 10px 10px;">${words.map(([w, c]) => `<div class="history-list-item" onclick="StatsManager.hide(); window.AppSearch('${UIUtils.escapeJS(w)}');"><div class="history-word-info"><span style="font-weight:600;">${w}</span><span class="history-word-count">(${c}x, ${this.getFormattedTime(this.stats.wordTime[w])})</span></div><button class="remove-history-btn" onclick="StatsManager.removeWord('${UIUtils.escapeJS(w)}', event)">&times;</button></div>`).join('')}</div>` : ''}
                ...
                ${this.currentOpenSection === 'tags' ? `<div class="stats-list" style="padding:0 10px 10px 10px;">${tags.map(([t, c]) => `<div class="history-list-item" onclick="StatsManager.hide(); window.ModalManager.show('${UIUtils.escapeJS(t)}');"><div class="history-word-info"><span style="font-weight:600;">${t}</span><span class="history-word-count">(${c}x, ${this.getFormattedTime(this.stats.tagTime[t])})</span></div><button class="remove-history-btn" onclick="StatsManager.removeTag('${UIUtils.escapeJS(t)}', event)">&times;</button></div>`).join('')}</div>` : ''}
            </div>
