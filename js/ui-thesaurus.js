window.UIThesaurus = {
    ...
    entry.def[0].sseq.forEach(sseq => {
        sseq.forEach(sen => {
            ...
            // Check if ANY tags exist before building the tags section
            const hasAnyTags = mergedSimilar.length || rels.length || mergedOpposite.length || nears.length;

            const tagsSectionHtml = hasAnyTags ? `
