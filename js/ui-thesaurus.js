                ...
                entry.def[0].sseq.forEach(sseq => {
                    sseq.forEach(node => {
                        const processNode = (n) => {
                            ...
                            const rows = [];
                            if (mergedSimilar.length) rows.push(`<div class="tags-row Similar-row"><span class="tags-label">similar:</span>${renderTags(mergedSimilar, 'syn-tag')}</div>`);
                            if (rels.length) rows.push(`<div class="tags-row"><span class="tags-label">related:</span>${renderTags(rels, 'syn-tag')}</div>`);
                            if (mergedOpposite.length) rows.push(`<div class="tags-row"><span class="tags-label">opposite:</span>${renderTags(mergedOpposite, 'ant-tag')}</div>`);
                            if (nears.length) rows.push(`<div class="tags-row"><span class="tags-label">near:</span>${renderTags(nears, 'ant-tag')}</div>`);
                            ...
                            if (rows.length > 0) {
                                ...
                                rows[0] = rows[0].replace(/<\/div>$/, `${expandBtnHtml}</div>`);
                            }

                            const hasTags = rows.length > 0;
