        ...
        let html = `
            <div class="list-page" style="padding: 20px 0;">
                ...
                <div class="tags-row" style="display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 30px; margin-top: 20px;">
                    ${pageWords.map(w => `<span class="tag syn-tag ${tagClass} ${window.UIUtils ? UIUtils.getTagClass(w) : ''}" data-word="${w}" tabindex="0" onclick="window.ModalManager.show('${UIUtils.escapeJS(w)}'); event.stopPropagation();">${w}</span>`).join('')}
                </div>
            </div>
        `;
