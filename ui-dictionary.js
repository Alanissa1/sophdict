                    ...
                    grouped[fl].forEach(e => {
                        ...
                        if (e.def && Array.isArray(e.def)) {
                                    ...
                                    defObj.sseq.forEach(sseq => {
                                        sseq.forEach(node => {
                                            this.processSenseNode(node, (itemHtml) => {
                                                const plainText = UIUtils.stripTags(itemHtml).trim();
                                                seenDefs.add(plainText);
                            ...
                            e.shortdef.forEach(d => {
                                ...
                                seenDefs.add(UIUtils.stripTags(d).trim());
                                sectionHtml += `
                                    ...
                                    if (defObj.sseq && Array.isArray(defObj.sseq)) {
                                        defObj.sseq.forEach(sseq => {
                                            sseq.forEach(node => {
                                                this.processSenseNode(node, (itemHtml) => {
                                                    const plainText = UIUtils.stripTags(itemHtml).trim();
                                                    // Only add if not seen and contains an example
                                                    if (!seenDefs.has(plainText) && itemHtml.includes('class="example"')) {
                                                        sectionHtml += itemHtml;
                                                        seenDefs.add(plainText);
                                                    }
                                                }, counter, true, word);
